...
test('assistance preview and reusable report task stay reviewed, inert, and local', async () => {
  ...
  try {
    ...
    await expect(assistance).toContainText('Harbor does not separately attach row data, parameter values, credentials, local paths or results.')
    await expect(assistance).toContainText('Redaction is best-effort; inspect the exact preview before sending.')
