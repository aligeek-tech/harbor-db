...
for (const engine of ['postgres', 'mariadb'] as const) {
  test(`${engine} table workbench tracks SQL, guards drafts, runs read-only results and reviews selected deletes`, async () => {
    ...
    try {
      ...
      await grid(page).getByRole('button', { name: 'Sort id ascending', exact: true }).click()
      await expect.poll(() => draft(page, table, profile.id)).toMatch(/ORDER BY.*id.*ASC/i)
      await grid(page).getByRole('button', { name: 'Sort id descending', exact: true }).click()
      ...
      await grid(page).getByRole('button', { name: 'Sort id descending', exact: true }).click()
      await expect(grid(page).locator('tbody tr[aria-rowindex]').first()).toContainText('row-002')
      await grid(page).getByRole('cell', { name: 'row-002', exact: true }).click()
      await expect(grid(page).getByRole('checkbox', { name: 'Select row 2', exact: true })).toBeChecked()
      await expect(grid(page).getByRole('checkbox', { name: 'Select row 1', exact: true })).not.toBeChecked()
      await grid(page).getByRole('button', { name: 'Sort id ascending', exact: true }).click()
      await expect(grid(page).locator('tbody tr[aria-rowindex]').first()).toContainText('row-001')
      await expect(grid(page).getByRole('checkbox', { name: 'Select row 2', exact: true })).toBeChecked()
